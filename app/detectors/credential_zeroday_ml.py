...
class CredentialAnomalyType(Enum):
    ...
    CONFIG_EMBEDDED_CREDENTIAL = "CONFIG_EMBEDDED_CREDENTIAL"
    JSON_API_KEY_EXPOSURE = "JSON_API_KEY_EXPOSURE"
...
class ConfigEmbeddedCredentialDetector:
    """Detects credentials embedded in config structures (JSON, YAML, env vars, etc.)"""
    
    # Credential key patterns in configs
    CONFIG_CREDENTIAL_KEYS = {
        "API_KEY": [
            r"(apiKey|api_key|API_KEY|accessKey|access_key)\s*[:=]\s*['\"]?([A-Za-z0-9_\-]{20,})['\"]?",
            r"(apiKey|api_key)\s*:\s*'([^']{20,})'",  # JSON format
        ],
        "FIREBASE_CONFIG": [
            r"(apiKey|authDomain|projectId|storageBucket|messagingSenderId|appId)\s*:\s*['\"]?([A-Za-z0-9_\-.]{10,})['\"]?",
        ],
        "AWS_CONFIG": [
            r"(AccessKeyId|SecretAccessKey|SessionToken)\s*[:=]\s*['\"]?([A-Za-z0-9/+]{20,})['\"]?",
        ],
        "GCP_CONFIG": [
            r"(type|project_id|private_key_id|private_key|client_email)\s*[:=]\s*['\"]?([^\s'\"]*)['\"]?",
        ],
        "DATABASE_CONFIG": [
            r"(password|passwd|pwd|secret)\s*[:=]\s*['\"]?([^\s'\"]{8,})['\"]?",
            r"(user|username|uid)\s*[:=]\s*['\"]?([a-zA-Z0-9_]{3,})['\"]?(?=\s*[:=].*password)",
        ],
        "JWT_BEARER": [
            r"(Authorization|authorization|auth)\s*[:=]\s*['\"]?Bearer\s+(eyJ[A-Za-z0-9_\-\.]+)['\"]?",
        ],
        "STRIPE_KEY": [
            r"(stripe_key|stripeKey|pk_live|sk_live)\s*[:=]\s*['\"]?([a-zA-Z0-9_]{40,})['\"]?",
        ],
        "GITHUB_TOKEN": [
            r"(github_token|GITHUB_TOKEN|gh_token)\s*[:=]\s*['\"]?([a-zA-Z0-9_]{40,})['\"]?",
        ],
        "SENDGRID_KEY": [
            r"(sendgrid|sendGrid|SG\.[a-zA-Z0-9_\-]{20,})",
        ],
    }
    
    def detect_config_credentials(self, text: str) -> List[CredentialAnomaly]:
        """Detect credentials embedded in configuration structures"""
        anomalies = []
        
        for config_type, patterns in self.CONFIG_CREDENTIAL_KEYS.items():
            for pattern in patterns:
                for match in re.finditer(pattern, text, re.IGNORECASE):
                    try:
                        # Extract the credential value (usually group 2 or last group)
                        groups = match.groups()
                        ...
                        if len(groups) >= 2:
                            key_name = groups[0]
                            credential_value = groups[-1]  # Last group is usually the value
                        
                        if credential_value and len(credential_value) >= 8:
                            confidence = self._calculate_config_confidence(credential_value, config_type)
                            
                            anomalies.append(CredentialAnomaly(
                                anomaly_type=CredentialAnomalyType.JSON_API_KEY_EXPOSURE if config_type in ["FIREBASE_CONFIG", "GCP_CONFIG", "API_KEY"] else CredentialAnomalyType.CONFIG_EMBEDDED_CREDENTIAL,
                                severity=self._get_config_severity(config_type),
                                confidence=confidence,
                                text_snippet=credential_value,
                                start=match.start(),
                                end=match.end(),
                                description=f"{config_type} credential exposed in config structure: key='{key_name}'",
                                analysis={
                                    "config_type": config_type,
                                    "key_name": key_name,
                                    "value_length": len(credential_value),
                                    "detected_pattern": pattern[:60],
                                },
                            ))
                    except Exception as e:
                        logger.debug(f"Config credential detection error: {e}")
        
        return anomalies
    
    def _calculate_config_confidence(self, credential_value: str, config_type: str) -> float:
        """Calculate confidence score for config credential"""
        base_score = 0.8  # Config credentials have high confidence
        
        # Boost for length and character diversity
        if len(credential_value) > 30:
            base_score += 0.1
        
        # Boost for mixed character types (not just lowercase)
        if re.search(r'[A-Z]', credential_value) and re.search(r'[0-9]', credential_value):
            base_score += 0.05
        
        # Boost for special characters
        if re.search(r'[_\-\.]', credential_value):
            base_score += 0.05
        
        # High confidence for known critical types
        if config_type in ["FIREBASE_CONFIG", "AWS_CONFIG", "GCP_CONFIG", "STRIPE_KEY", "GITHUB_TOKEN"]:
            base_score = min(base_score + 0.15, 1.0)
        
        return min(base_score, 1.0)
    
    def _get_config_severity(self, config_type: str) -> ThreatSeverity:
        """Determine severity based on config type"""
        critical_types = ["FIREBASE_CONFIG", "AWS_CONFIG", "GCP_CONFIG", "STRIPE_KEY", "GITHUB_TOKEN", "SENDGRID_KEY"]
        high_types = ["DATABASE_CONFIG", "JWT_BEARER"]
        
        if config_type in critical_types:
            return ThreatSeverity.CRITICAL
        elif config_type in high_types:
...
class NeuralCredentialAnomalyDetector(nn.Module):
    ...
    def __init__(self, input_dim: int = 256, hidden_dim: int = 128, num_anomaly_types: int = 10):
        super().__init__()
...
class CredentialZeroDayDetector:
    ...
    Combines entropy analysis, polymorphic detection, config extraction, and neural anomaly detection.
    """
    ...
    def __init__(self):
        ...
        self.config_detector = ConfigEmbeddedCredentialDetector()
        ...
        self.anomaly_type_to_severity = {
            ...
            CredentialAnomalyType.CONFIG_EMBEDDED_CREDENTIAL: ThreatSeverity.HIGH,
            CredentialAnomalyType.JSON_API_KEY_EXPOSURE: ThreatSeverity.CRITICAL,
        }
    ...
    def detect_zeroday_anomalies(self, text: str) -> List[CredentialAnomaly]:
        ...
        # 1. Config-embedded credentials (HIGH PRIORITY - catches Firebase, GCP, etc.)
        config_anomalies = self.config_detector.detect_config_credentials(text)
    ...
    def _detect_credential_spray(self, text: str) -> List[CredentialAnomaly]:
        ...
        # Count potential credential tokens + config credentials
        potential_tokens = self._extract_potential_tokens(text)
        config_creds = self.config_detector.detect_config_credentials(text)
        total_creds = len(potential_tokens) + len(config_creds)
        
        if total_creds > 5:  # Threshold for spray
            return [CredentialAnomaly(
                anomaly_type=CredentialAnomalyType.CREDENTIAL_SPRAY_PATTERN,
                severity=ThreatSeverity.CRITICAL,
                confidence=min(total_creds / 20.0, 1.0),
                text_snippet=text[:100],
                start=0,
                end=min(100, len(text)),
                description=f"Credential spray pattern: {total_creds} potential credentials in single request",
                analysis={"credential_count": total_creds, "config_creds": len(config_creds)},
            )]
